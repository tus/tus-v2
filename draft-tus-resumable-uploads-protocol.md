...
The client MAY send the metadata of the file using headers such as `Content-Type` (see {{Section 8.3 of HTTP}} and `Content-Disposition` {{!RFC6266}} when starting a new upload. If so, the client MUST only include these header fields in the initial Upload Transfer Procedure, which starts the new upload. The server MUST ignore these header fields in subsequent Upload Transfer Procedures. The `Content-Type` header MAY include the MIME type of the file. The `Content-Disposition` header MAY include the `filename`, `filename*` or `boundary` parameters. The server MAY interpret this data or MAY choose to ignore it. 
